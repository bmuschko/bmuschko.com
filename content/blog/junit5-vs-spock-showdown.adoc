...
For capturing more complex expressions, the JUnit API also allows for writing an implementation of link:http://junit.org/junit5/docs/current/user-guide/#extensions-conditions[`ExecutionCondition`]. The interface represents an extension to the test framework and can only be applied on the class-level which makes its use somewhat limiting. You can find an example for an `ExecutionCondition` in the source code on GitHub. For more information about <<extending-test-framework,extending the test framework>>, see the section below.
