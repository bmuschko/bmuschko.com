...
Production source code might behave differently at runtime if provided with varying inputs. Copy-pasting the same test case over and over again but using different data leads to unmaintainable test classes. The better option is to run the same test case multiple times but with different inputs. I personally use this feature in almost every project I am working on and wouldn't want to miss it.
...
IntelliJ groups data-driven execution per test method making it very readable. The IDE also renders the type for each input value in parathesis next to the test name.
...
Spock solves data-driven test in a very elegant way. First of all, the data is provided as a table with descriptive headers in a `where` clause. Each line in the data table represents a single execution of the test making all of the values available with the corresponding named identifier. Each named identifier can be referenced in the method name with the pound character (`#`) to build a meaningful test case description. The `@Unroll` annotation tells Spock to treat each test execution as individual test case. From my perspective data-driven tests are the Spock killer feature.
...
Classes rarely work in isolation. Most classes call off to other class instances, services or subsystem. link:https://en.wikipedia.org/wiki/Mock_object[Mocking] lets you cut off those integration points, replace it by a stand-in object and make it respond as needed for the test case.

=== JUnit 5

JUnit 5 does not come with any mocking capabilities. Many users resort to the popular mocking library link:http://site.mockito.org/[Mockito]. The following example demonstrates the use of Mockito as part of a test case. As you can see the Mockito API calls nicely blend into the test cases logic. Beginners to testing might have a hard time at first to visually separate test code from mocking expectations.
...
Spock ships with an API for creating mock objects as well as stubs. It's appealing that you do not have to pull in yet another external dependency to fulfill all the needs of your project. From my experience mock objects in Spock work great and are very readable. You can learn more about Spock's link:http://spockframework.org/spock/docs/1.0/interaction_based_testing.html[interaction based testing] in the user guide.
...
With an increasing number of tests you'll want to group test with a common purpose and run them separately. For example you might want to split slow- from fast-running tests, unit from integration tests or set up a test suite for load testing. Both test frameworks provide functionality for labeling tests and running them in dedicated test runs.
...
Tagging tests is not very helpful by itself. You might give an indication to other developers about the purpose or runtime behavior of the test. However, there's no special treatment for tagged tests in JUnit. Only in combination with test filtering, tagging becomes really useful. The following example sets up a test suite for running "fast" tests only. Filtering tests requires the dependency `org.junit.platform:junit-platform-runner` to be available on the test compilation classpath.
...
The Spock API does not define the concept of a tag. However, it's very easy to set it up yourself. The following two annotation classes serve the purpose of implying slow- and fast-running tests. You can put the annotation classes anywhere on the classpath with your test classes.
...
Labeling test case methods with the annotations is straight forward. The example below uses both annotations. Using types to tag test methods also prevents misspelling the label.
...
Labeled test cases can be filtered with the help of a Spock configuration file. Spock searches for a file with the name `SpockConfig.groovy` on the classpath. The configuration file can tell the test runner which test suite to execute. The approach it less convenient than the one provided by JUnit 5 especially if you have more than one test suite that you want to run independently on a regular basis from the IDE. Please be aware that you can also provide the configuration to the test JVM with the system property `spock.configuration`.
...
JUnit 5 and Spock are powerful tools. Nevertheless, a test framework cannot foresee all potential use cases out there. You can enhance the base functionality with an extension in case you need custom logic that ties into the API. This blog post won't go very deep into the available options for each test framework. I'd encourage you to explore the API yourself.

The simple example below demonstrates the extension capabilities. The implementation reacts to lifecycle events emitted before and after executing a test case. Each event logs the name of test method to the standard output stream.

=== JUnit 5

The test framework's link:http://junit.org/junit5/docs/current/user-guide/#extensions[extension model] provides callback interfaces that allow you to react to certain events. As you can see in listing 30, the extension class implements `BeforeTestExecutionCallback` and `AfterTestExecutionCallback`. The callback interface provide a context object of type `ExtensionContext` giving you access to information about the test method.
...
In our example, the extension should only be available for the test class. Listing 33 implements annotation class for that purpose.
