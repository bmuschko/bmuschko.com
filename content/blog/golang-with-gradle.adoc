...
Testing support for Go follows the same conventions you might know from Java-based projects. The `test` task provided by the plugin finds all Go file that follow the convention `_test.go`. GoGradle properly detects and runs all tests in the project as shown in the following console output:
...
By default the task automatically generates test reports as well as coverage profile data which can be found in the directory `.gogradle/reports`. Figure 1 shows a sample report for the project. 

image:/img/blog/golang-with-gradle/test-report.png[Test report]
_Figure 1. HTML test report_

At the time of writing the plugin creates one coverage profile data file per Go test file. Aggregating individual coverage files into one file would require some custom logic. Listing 5 demonstrate a simple solution to the problem though I expect this functionality to be worked into the plugin sooner or later.

_buildSrc/src/main/groovy/AggregateCoverage.groovy_
[source,groovy]
----
...
After exploring the plugin's testing capabilities, let's finish off the discussion by having a look a the cross-compilation functionality.
...
The task creates the resulting prebuilt libraries in the directory `.gogradle`. Each file name consists of the provided target platform and the name of the project. Currently, the version assigned to the project is not taken into account.
...
In my shell script I also extracted all third-party licenses and bundled them with the binary in a `tar.gz` or `.zip` file. The plugin does not provide this functionality. However, it's very easy to achieve this with the help of Gradle's built-in capabilities.

_buildSrc/src/main/groovy/Distributions.groovy_
[source,groovy]
----
import org.gradle.api.DefaultTask
