...
The console output in verbose mode looks fairly bland but does the job.
...
The testing framework gocheck condenses the usual test results in the console output a bit more. It's unfortunate that the level of details cannot be controlled by a command line option.
...
Ginkgo really shines with its included assertion package, link:http://onsi.github.io/gomega/[Gomega]. It allows for writing a builder-style assertion logic that could even be used outside of Gingko. It reads like natural language and is easy to write. Additionally, Gomega provides a shortcut symbol for writing assertion via the `Ω` notation. The notation may align well from a branding perspective but I am personally not a fan of using it as it makes the syntax more obscure and harder to parse.
...
Good reporting is a must-have for projects with a large test suite. link:http://goconvey.co/[GoConvey] is another BDD-style testing framework with a similar API to the one defined by Goblin. See listing 8 for a short example.
...
While many Go developers are happy with the standard testing support, the Go ecosystem produced a wealth of testing libraries that go far beyond the basics. You can avoid writing too much boilerplate code with the help of an assertion library. My person favorite is Testify.

BDD-style testing frameworks can improve the expressiveness of your test code. I do like Goblin for its simplicity and clearly represented console output. If I'd need a richer feature set, I'd probably go with Ginkgo.
