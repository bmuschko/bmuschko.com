...
Identifying the root cause of a failed assertion can be tedious and may require additional debugging. A power assertion library helps with rendering the evaluated result of every portion of the equation as part of the error message. The library link:https://github.com/ToQoz/gopwtp[gopwt] exposes a single function named `OK` for asserting an expression as shown in listing 5. You may have noticed that the test function `TestAddWithGopwt` expects an incorrect result and therefore should fail.
